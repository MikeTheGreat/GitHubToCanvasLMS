...
| `course_settings/rubrics.toml` | Each rubric created or updated in place (matched by title) via `course.create_rubric()` / `PUT rubrics/:id`; supports `long_description`, `reusable`, `read_only`. Per-rubric change detection via the manifest entry's `rubric_hashes` sub-table (see below) |
