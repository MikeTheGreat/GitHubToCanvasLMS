...
**Future work** (spec'd in `DESIGN-course-flags.md` §12, tracked in TODO.md): whole-resource exclusion (`only_if:` frontmatter), `--flag` CLI overrides, due_dates value caching on the same per-file-values principle, richer conditions (`and`/`or`, non-boolean values), and a `list-flags` report.
