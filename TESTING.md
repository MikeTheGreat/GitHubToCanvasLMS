...
- **Ignore files** (`test_ignore.py`): `load_ignore_matcher`/`IgnoreMatcher` — empty (no ignore files) matches nothing, `.gitignore` pattern matching (globs, `~$*`, directory-only `build/`), `.canvasignore` layering on top of `.gitignore`, git-style `!` negation, and the always-ignored `.canvas-manifest.toml`. Integration coverage in `test_sync.py` asserts an ignored stray asset isn't uploaded (and is when no ignore file is present) and an ignored content file isn't synced.
